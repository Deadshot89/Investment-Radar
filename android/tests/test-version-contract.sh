...
# Final Integration Contract für Investment Radar 2.5.6 / Android 2.5.6.
...
grep -Fq 'versionCode = 76' "$GRADLE_FILE" || fail 'Android versionCode muss 76 sein.'
grep -Fq 'versionName = "2.5.6"' "$GRADLE_FILE" || fail 'Android versionName muss 2.5.6 sein.'
grep -Fq 'Release candidate: Investment Radar 2.5.6' "$GRADLE_FILE" || fail 'Release-Kandidat muss 2.5.6 benennen.'
...
if grep -Fq 'versionCode = 74' "$GRADLE_FILE"; then
  fail 'Alter versionCode 74 darf im Release-Kandidaten nicht mehr aktiv sein.'
fi
if grep -Fq 'versionName = "2.5.4"' "$GRADLE_FILE"; then
  fail 'Alte versionName 2.5.4 darf im Release-Kandidaten nicht mehr aktiv sein.'
fi
for temp_artifact in "$TEMP_NAV_WORKFLOW" "$TEMP_DETAIL_WORKFLOW" "$TEMP_TASK10_WORKFLOW" "$TEMP_MONEY_WORKFLOW" "$TEMP_MONEY_SCRIPT" "$TEMP_PREFILL_WORKFLOW" "$TEMP_PREFILL_SCRIPT"; do
  if [ -e "$temp_artifact" ]; then
    fail "Temporäres Reparaturartefakt darf im Release-Kandidaten nicht enthalten sein: $temp_artifact"
  fi
done

echo 'PASS: Android 2.5.6/code76 mit Backend-Vertrag 2.1.0 und bereinigtem Release-Branch.'
