#!/usr/bin/env bash
# Investment Radar 2.5.1 release verification: backend compatibility, 2000-item radar and monotonic Android update.
...
# The live backend contract remains 2.1.0; Android 2.5.1/code71 is the new update candidate.
grep -q 'versionCode = 71' "$GRADLE"
grep -q 'versionName = "2.5.1"' "$GRADLE"
grep -q 'Investment Radar 2.5.1' "$GRADLE"
...
echo "PASS Android app release is monotonic at 2.5.1 / code 71"
