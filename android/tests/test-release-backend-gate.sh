#!/usr/bin/env bash
# Investment Radar 2.4 release verification: backend compatibility, 2000-item radar and monotonic Android update.
...
# The live backend contract remains 2.1.0; Android 2.4.0/code60 is the new update candidate.
grep -q 'versionCode = 60' "$GRADLE"
grep -q 'versionName = "2.4.0"' "$GRADLE"
grep -q 'Investment Radar 2.4.0' "$GRADLE"
...
echo "PASS Android app release is monotonic at 2.4.0 / code 60"
