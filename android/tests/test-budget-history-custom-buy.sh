...
grep -Fq 'fun from(summary: InvestmentBudgetSummary, entries: List<BudgetJournalEntry>)' "$STATE" || fail 'BudgetViewState muss aus Summary und Journal-Einträgen aufgebaut werden.'
...
grep -Fq 'budgetState.history' "$UI" || fail 'Die Budget-Historie wird in der Oberfläche nicht gerendert.'
