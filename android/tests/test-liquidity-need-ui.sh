...
grep -Fq 'Text("ICH BRAUCHE GELD")' "$MAIN" || fail 'Geldbedarf-Funktion fehlt.'
grep -Fq 'Text("Freies Cash zuerst verwenden")' "$MAIN" || fail 'Cash-zuerst-Schalter fehlt.'
grep -Fq 'LiquidityNeedEngine.plan(' "$MAIN" || fail 'Geldbedarf wird nicht berechnet.'
grep -Fq 'Text("Auszahlung verbuchen")' "$MAIN" || fail 'Auszahlung kann nicht gebucht werden.'
