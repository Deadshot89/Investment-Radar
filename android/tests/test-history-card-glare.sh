...
# The glare fix remains present in the current Android 2.5.6 release candidate.
if ! grep -q 'versionCode = 76' "$BUILD" || ! grep -q 'versionName = "2.5.6"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.6 (code 76)"
  exit 1
fi

echo "History card glare and Android 2.5.6 release contract OK"
