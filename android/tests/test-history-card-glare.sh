...
# The glare fix remains present in the current Android 2.4 release candidate.
if ! grep -q 'versionCode = 60' "$BUILD" || ! grep -q 'versionName = "2.4.0"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.4.0 (code 60)"
  exit 1
fi

echo "History card glare and Android 2.4.0 release contract OK"
