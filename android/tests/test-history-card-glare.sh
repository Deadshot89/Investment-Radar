...
# The glare fix remains present in the current Android 2.4.9 release candidate.
if ! grep -q 'versionCode = 69' "$BUILD" || ! grep -q 'versionName = "2.4.9"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.4.9 (code 69)"
  exit 1
fi

echo "History card glare and Android 2.4.9 release contract OK"
