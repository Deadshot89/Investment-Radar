...
# The glare fix remains present in the current Android 2.5.15 release candidate.
if ! grep -q 'versionCode = 85' "$BUILD" || ! grep -q 'versionName = "2.5.15"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.15 (code 85)"
  exit 1
fi

echo "History card glare and Android 2.5.15 release contract OK"
