...
if ! grep -q 'versionCode = 59' "$BUILD" || ! grep -q 'versionName = "2.1.6"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.1.6 (code 59)"
  exit 1
fi

echo "History card glare and Android 2.1.6 release contract OK"
