...
# The glare fix remains present in the current Android 2.5.4 release candidate.
if ! grep -q 'versionCode = 74' "$BUILD" || ! grep -q 'versionName = "2.5.4"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.4 (code 74)"
  exit 1
fi

echo "History card glare and Android 2.5.4 release contract OK"
