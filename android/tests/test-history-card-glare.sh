...
# The glare fix remains present in the current Android 2.5.23 release candidate.
if ! grep -q 'versionCode = 93' "$BUILD" || ! grep -q 'versionName = "2.5.23"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.23 (code 93)"
  exit 1
fi

echo "History card glare and Android 2.5.23 release contract OK"
