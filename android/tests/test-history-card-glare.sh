...
# The glare fix remains present in the current Android 2.5.18 release candidate.
if ! grep -q 'versionCode = 88' "$BUILD" || ! grep -q 'versionName = "2.5.18"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.18 (code 88)"
  exit 1
fi

echo "History card glare and Android 2.5.18 release contract OK"
