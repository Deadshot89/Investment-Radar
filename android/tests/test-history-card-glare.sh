...
# The glare fix remains present in the current Android 2.5.3 release candidate.
if ! grep -q 'versionCode = 73' "$BUILD" || ! grep -q 'versionName = "2.5.3"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.3 (code 73)"
  exit 1
fi

echo "History card glare and Android 2.5.3 release contract OK"
