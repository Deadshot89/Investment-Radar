...
# The glare fix remains present in the current Android 2.5.12 release candidate.
if ! grep -q 'versionCode = 82' "$BUILD" || ! grep -q 'versionName = "2.5.12"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.12 (code 82)"
  exit 1
fi

echo "History card glare and Android 2.5.12 release contract OK"
