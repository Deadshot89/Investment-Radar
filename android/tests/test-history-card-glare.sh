...
# The glare fix remains present in the current Android 2.5.0 release candidate.
if ! grep -q 'versionCode = 70' "$BUILD" || ! grep -q 'versionName = "2.5.0"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.0 (code 70)"
  exit 1
fi

echo "History card glare and Android 2.5.0 release contract OK"
