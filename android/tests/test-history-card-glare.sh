...
# The glare fix remains present in the current Android 2.4.1 release candidate.
if ! grep -q 'versionCode = 61' "$BUILD" || ! grep -q 'versionName = "2.4.1"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.4.1 (code 61)"
  exit 1
fi

echo "History card glare and Android 2.4.1 release contract OK"
