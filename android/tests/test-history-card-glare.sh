...
# The glare fix remains present in the current Android 2.5.14 release candidate.
if ! grep -q 'versionCode = 84' "$BUILD" || ! grep -q 'versionName = "2.5.14"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.14 (code 84)"
  exit 1
fi

echo "History card glare and Android 2.5.14 release contract OK"
