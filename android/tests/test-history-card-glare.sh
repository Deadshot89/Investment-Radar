...
# The glare fix remains present in the current Android 2.5.24 release candidate.
if ! grep -q 'versionCode = 94' "$BUILD" || ! grep -q 'versionName = "2.5.24"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.24 (code 94)"
  exit 1
fi

echo "History card glare and Android 2.5.24 release contract OK"
