...
# The glare fix remains present in the current Android 2.5.13 release candidate.
if ! grep -q 'versionCode = 83' "$BUILD" || ! grep -q 'versionName = "2.5.13"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.13 (code 83)"
  exit 1
fi

echo "History card glare and Android 2.5.13 release contract OK"
