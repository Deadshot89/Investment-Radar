...
# The glare fix remains present in the current Android 2.5.22 release candidate.
if ! grep -q 'versionCode = 92' "$BUILD" || ! grep -q 'versionName = "2.5.22"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.22 (code 92)"
  exit 1
fi

echo "History card glare and Android 2.5.22 release contract OK"
