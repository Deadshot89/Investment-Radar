...
# The glare fix remains present in the current Android 2.5.5 release candidate.
if ! grep -q 'versionCode = 75' "$BUILD" || ! grep -q 'versionName = "2.5.5"' "$BUILD"; then
  echo "Expected the current release candidate to be Android 2.5.5 (code 75)"
  exit 1
fi

echo "History card glare and Android 2.5.5 release contract OK"
