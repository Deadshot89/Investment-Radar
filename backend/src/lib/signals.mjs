...
export function evaluateSignals(items, quotes = new Map(), context = {}) {
  ...
  for (const item of items) {
    ...
    if (price != null && item.hardReviewBelow != null && price <= Number(item.hardReviewBelow)) {
      result.push(make(item, "THRESHOLD", `${item.name}: Kurs-Schwelle erreicht`, `Kurs ${fmt(price)} ${currency} liegt unter der Prüfschwelle ${fmt(Number(item.hardReviewBelow))}. Kein automatischer Verkauf – Gründe prüfen.`, now, `below-${item.hardReviewBelow}`));
    }
    ...
    if (previousForecast && previousForecast.direction && previousForecast.direction !== currentForecast.direction) {
      ...
      const title = currentForecast.direction === "DOWN" ? `${item.name}: 12M-Prognose kippt nach unten` : currentForecast.direction === "UP" ? `${item.name}: 12M-Prognose dreht nach oben` : `${item.name}: 12M-Prognose wird neutral`;
      result.push(make(
        item, level, title,
        `12M-Prognose: ${from}${prior != null ? ` (${signed1(prior)} %)` : ""} → ${to} (${signed1(current)} %). Pessimistisch: ${signed1(currentForecast.bearChangePct)} %. Erwartet: ${signed1(currentForecast.expectedChangePct)} %. Optimistisch: ${signed1(currentForecast.bullChangePct)} %. Warum: ${currentForecast.reasons.join(" ")}`,
        now, `forecast-direction-${previousForecast.direction}-${currentForecast.direction}-${Math.round(current * 10)}`
      ));
    }
    ...
    if (held && currentScore != null && previousScore != null && previousScore - currentScore >= 15) {
      result.push(make(item, "REVIEW", `${item.name}: Score deutlich gefallen`, `Der Investment-Score ist von ${Math.round(previousScore)} auf ${Math.round(currentScore)} gefallen. Prüfe Bewertung, Wachstum und Investmentthese.`, now, `score-drop-${Math.round(previousScore)}-${Math.round(currentScore)}`));
    }
    if (held && currentScore != null && currentScore < 45) {
      result.push(make(item, "REVIEW", `${item.name}: schwacher Gesamtscore`, `Der aktuelle Gesamtscore liegt nur bei ${Math.round(currentScore)}/100. Position und Investmentthese prüfen.`, now, "score-review-floor-45"));
    }

    const m3 = finite(item?.momentum?.m3);
    const m6 = finite(item?.momentum?.m6);
    if (held && m3 != null && m6 != null && m3 <= -10 && m6 <= -10) {
      result.push(make(item, "REVIEW", `${item.name}: mittelfristiger Trendbruch`, `3M (${m3.toFixed(1)} %) und 6M (${m6.toFixed(1)} %) sind gleichzeitig deutlich negativ.`, now, "momentum-breakdown-m3-m6"));
    }
    ...
    if (held && fundamentalDataSufficient && revenueGrowth != null && epsGrowth != null && revenueGrowth <= -0.10 && epsGrowth <= -0.20) {
      result.push(make(item, "REVIEW", `${item.name}: fundamentale Verschlechterung`, "Umsatz- und Gewinnwachstum sind gleichzeitig deutlich negativ. Investmentthese prüfen.", now, "fundamental-growth-breakdown"));
    }
    ...
    if (recommendation === "BUY" && previousRecommendation && previousRecommendation !== "BUY") {
      result.push(make(item, "BUY", `${item.name}: neue Kaufchance`, `Der Radar ist von ${previousRecommendation} auf BUY gewechselt${currentScore != null ? ` (Score ${Math.round(currentScore)}/100)` : ""}. 12M-Prognose: ${directionLabel(currentForecast.direction)} (${signed1(currentForecast.expectedChangePct)} %). Warum: ${currentForecast.reasons.join(" ")}`, now, `buy-transition-${previousRecommendation}-BUY`));
    }
  }
...
function movementReason(item, percentChange) {
  ...
  const m3 = finite(item?.momentum?.m3);
  if (m1 != null && Math.sign(m1) === Math.sign(percentChange) && Math.abs(m1) >= 5) return `Die Bewegung bestätigt das 1M-Momentum (${signed1(m1)} %). Nachrichten und Fundamentaldaten trotzdem prüfen.`;
  if (m3 != null && Math.sign(m3) === Math.sign(percentChange) && Math.abs(m3) >= 5) return `Die Bewegung passt zum 3M-Trend (${signed1(m3)} %). Nachrichten und Fundamentaldaten prüfen.`;
...
function dedupeByFingerprint(alerts) { const seen = new Set(); return alerts.filter((a) => !seen.has(a.fingerprint) && seen.add(a.fingerprint)); }
function finite(value) { if (value == null || (typeof value === "string" && value.trim() === "")) return null; const n = typeof value === "number" ? value : Number(value); return Number.isFinite(n) ? n : null; }
